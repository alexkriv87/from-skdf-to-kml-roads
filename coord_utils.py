...
def parse_coordinate(coord_str):
    ...
    # ==================== ДЕСЯТИЧНЫЙ ФОРМАТ (Яндекс.Карты) ====================
        ...
        if not parts or len(parts) < 2:
            ...
            raise ValueError(
                f"Неверный формат десятичных координат: {coord_str}")
...
def build_bbox(point1, point2):
    ...
    logger.debug(
        f"Bbox: west={west}, south={south}, east={east}, north={north}")
...
def convert_bbox_to_skdf(bbox_degrees):
    ...
    logger.debug(
        f"Bbox в метрах: xmin={xmin}, ymin={ymin}, xmax={xmax}, ymax={ymax}")
